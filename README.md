...
`cooperative_join_n` is **adaptive on N**, against the pool width rather than against constants. Below the worker count it builds a balanced binary tree of `sched::join` (depth `log2(N)`). From there to 32 times the worker count it uses a flat fan-out of N StackJobs (depth 1) onto the calling worker's own deque, which random peer-steal distributes. At or above 32 times the worker count the flat fan-out switches to owner-directed placement, pushing each closure to one specific worker's mailbox. The tree amortizes per-StackJob setup across the bisect (wins at short closures, e.g. MIMC pivoted-LU N=8 at ~17 us); the flat fan-out trims the critical-path nesting the tree's `log2(N)` levels add to the slowest closure (wins at multi-millisecond closures at SMT-pool saturation).

The 32x gate is measured, on a 24-worker Zen 4 with every width run in both registration orders, comparing the deque fan-out against the routed entry point a caller reaches: deque is faster at N = 24, 64, 256, 512 and 640, and the routed shape at 768, 896 and 1024, putting the crossing between 640 and 768. Callers whose workload breaks the assumption can call [`cooperative_join_n_tree`](src/sched/cooperative.rs) or [`cooperative_join_n_flat`](src/sched/cooperative.rs) directly, and [`cooperative_join_n_flat_mailbox`](src/sched/cooperative.rs) for owner-directed placement at any width.
...
```toml
flynnel = { version = "0.5", default-features = false, features = ["verify-chain"] }
```
