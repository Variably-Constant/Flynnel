...
Without an explicit estimate two costs apply per call that an explicit one removes: the total is unknown, so the call takes the JEC wake path rather than the sub-200 us polling path, and the inline-collapse below 50 us of total work never fires. The collapse threshold is one of three values in this host's measured dispatch profile, `par_iter::host_dispatch_profile` (`HostDispatchProfile { dispatch_cost_ns, collapse_threshold_ns, jec_wake_threshold_ns }`), measured once per process by the first query (13 to 20 ms on the Ryzen 7 2700) or earlier by `par_iter::calibrate_host_dispatch` at start-up; no unmeasured value is ever handed out. The calibration times a compute-bound body (eight dependent multiply-rotate steps per item) serial against dispatched through the pool, medians of five each, after sixteen discarded warm-up dispatches: once at two leaves for the dispatch cost (dispatched minus serial); over doubling counts from four leaves for the collapse crossover, the serial time at which the dispatched body finishes no later, interpolated between the last two counts, five sweeps, median, floored at the dispatch cost; and the same doubling sweep dispatched under the polling scope against the wake scope for the wake-path crossover. A sweep whose second side never catches up reports the serial time at the largest count swept, so a loaded host collapses more inline and wakes later, never the reverse. Readers of the profile: the collapse in every walker (`collapse_threshold_ns`), the tier pick's heavy-item override (the same value), the leaf sizing from an explicit or probed per-item cost (`dispatch_cost_ns` of work per leaf, capped at the leaf floor), the probe's decision to dispatch its tail (`collapse_threshold_ns`), and the switch between polling and the sleep-counter wake path (`jec_wake_threshold_ns`). Measured on the Ryzen 7 2700 across six processes: dispatch cost 4.9 to 12.7 us, collapse threshold 16.0 to 30.2 us, wake threshold 14.0 to 19.3 us; on the Ryzen 9 7900X (2026-09-05 16:33, one other process on one thread): collapse threshold 14.4 us. The body is compute-bound on purpose: a streaming one-add body measured the memory system instead, and on the Ryzen 9 7900X the pool never beat a serial stream below the cap, which would have collapsed 100000 items of 6 ns compute (717 us serial) that the pool runs in 87 us. Measured: the floor, 50 us, on the Ryzen 7 2700 in three processes (9 to 13 ms each) and on the Ryzen 9 7900X, consistent on the 2700 with 1000 items of 10 ns compute costing 16 us dispatched against 11 us serial and 10000 costing 44 to 70 us against 114. Memory-streaming work sits differently on that host: 100000 slice adds (33 us serial) cost 65 to 75 us dispatched, so above the threshold such a body pays the pool. The estimate is a whole number of nanoseconds per item, so an item under a nanosecond is stated as one and its total overstated accordingly. The collapse applies in `for_each_chunk`, `for_each_chunk_triple` and `for_each_chunk_triple_min_leaf`, and `for_each_chunk_indexed` and `for_each_chunk_indexed_min_leaf` (so also `for_each_indexed` and `for_each_chunk_ref`, which are built on it): the body runs once on the calling thread. Measured through the triple entry with a 1 ns per-item estimate (medians of 300 calls), registry 0.2.2 against this code. Ryzen 7 2700: 1000 items 0.5 us against 0.4 serial and 13.7 us in 0.2.2; 10000 items 4.4 us against 3.1 serial and 37.7 in 0.2.2; 100000 items, above the threshold, 75.5 us dispatched against 32.6 serial and 71.7 in 0.2.2. Ryzen 9 7900X (idle, 2026-09-05 14:39): 1000 items 0.3 us against 0.1 serial and 5.1 in 0.2.2; 10000 items 1.5 us against 1.5 serial and 14.2 in 0.2.2; 100000 items 32.9 us dispatched against 17.3 serial and 33.4 in 0.2.2. Measured on a Ryzen 7 2700 (16 threads, medians of 300 calls): a few-flop item at n = 10000 takes 28.7 us with `JobPlan::new`, 30.1 us with `set_profile`, 20.9 us with `with_estimated_per_item_ns`, 26.2 us under rayon, 17.8 us serial; a 10 ns item at n = 1000 takes 16.9 / 12.5 / 11.7 / 22.0 / 11.5 us; a 600 ns item at n = 1000 takes 125 / 98.7 / 98.3 / 133 / 599 us, and 82.4 us with both the profile and the estimate.

### `spin_before_yield_ns: Option<u64>`
