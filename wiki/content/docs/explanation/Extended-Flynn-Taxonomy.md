...
[`cooperative_join_n(plan, closures)`](Sched-Module-Reference.md#cooperative_join_n) takes a list of N closures and runs them under a single sync boundary. Unlike MIMD `join` (which treats the two halves as independent tasks), the cooperative path commits to running all N together and returns only once every one has finished, so the closures can compute partial results that combine deterministically. The cooperative shape matches reductions across a cache-coherent complex (intra-CCX on AMD Zen, intra-cluster on Apple silicon).

N counts closures, not cores. It may exceed the pool width, and the execution shape depends on both: below the worker count the call bisects as a tree; at or above it the flat fan-out pushes onto the calling worker's own deque and random peer-steal distributes; only at or above 32 times the worker count does it switch to owner-directed mailbox placement, which is the one shape that targets a worker per closure. The result order is deterministic given N in all three. The placement is not, so a closure must not depend on landing on a particular worker or on any two closures running at the same time.

### MIMC: heterogeneous roles inside one cooperative call

When the closures passed to `cooperative_join_n` are not identical, the call is MIMC (Multiple Instruction, Multiple Cores). The classic case: in a 4-way cooperative reduce, three closures compute partials and the fourth computes a calibration probe, and the result combines deterministically. All four are dispatched together and the call waits for all four, which is what makes the combination well defined; whether any two of them overlap in time is up to the shape the width selects and the peers available to steal. Flynnel's `K_class` axis names this pattern (`K_class = 2` for two distinct roles, etc.); the cooperative_join_n primitive carries the partition shape directly.
